...
## 4. fasta2primer3.sh
This script uses the RAD loci FASTA file (all_selected_RADloci.fa) outputed by selected_loci_fasta_vcf.sh to get the sequences that will be used as template to design primers by primer3 ([https://github.com/primer3-org/primer3]) for GT-seq. First it obtains two fasta files, one with the complete RAD loci sequences and another with the sequences of the GT-seq loci (target regions) that meets the the criteria to degign primers for GT-seq. Those sequences are selected based on two bed files with corresponding intervals (RAD loci and GT-seq loci).
