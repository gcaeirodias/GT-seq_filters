...
process_discarded_loci() {
    ...
    bedtools intersect -wa -a "$WORK_DIR/RADloci_SNPlist_tab.bed" -b "$dir/discarded_loci.bed" > "$dir/discarded_RADloci_SNPlist.bed"
    tr ',' '\t' < "$dir/discarded_RADloci_SNPlist.bed" > "$dir/discarded_RADloci_SNPlist_2.bed"

    # Process by excluding last and first SNPs
    process_exclusions "$dir" "last"
    process_exclusions "$dir" "first"
...
process_exclusions() {
    ...
    # Step 4: Remove loci with negative distances (only one SNP)
...
# Similar pattern as second batch
...
cat "selected_GTseq_loci.bed" "recovered_GTseq_loci.bed" | sort -n -k1,1 -k2,2 -k3,3 > "all_selected_GTseq_loci.bed"
cat "selected_RADloci.bed" "recovered_RAD_loci.bed" | sort -n -k1,1 -k2,2 -k3,3 > "all_selected_RADloci.bed"
